...
# 특정 컬럼값 일괄 변경 (조건부 연산)
df1['SpecialColumn'] = df1['SpecialColumn'].apply(lambda x: x * 2 if x > 10 else x * 3)
df2['SpecialColumn'] = df2['SpecialColumn'].apply(lambda x: x * 2 if x > 10 else x * 3)
df3['SpecialColumn'] = df3['SpecialColumn'].apply(lambda x: x * 2 if x > 10 else x * 3)
